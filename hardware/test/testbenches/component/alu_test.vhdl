...
--! @brief Default alu test behavior.
--! @details Tests the alu component given the input csv file.
...
signal a_in_s:std_logic_vector(32 - 1 downto 0);--! ALU data input A.
signal b_in_s:std_logic_vector(32 - 1 downto 0);--! ALU data input B.
signal carry_in_s:std_logic;--! ALU carry in.
signal function_in_s:std_logic_vector( 3 downto 0);--! ALU function selector.
signal c_out_s:std_logic_vector(32 - 1 downto 0);--! ALU result output C.
signal status_out_s:std_logic_vector( 4 downto 0);--! ALU status output.
...
--------------------------------------
    ...
    constant PATH_FILE_IN  : string := "../test/in/component/alu_test.csv"; --! Input file path.
    constant PATH_FILE_OUT : string := "../test/out/component_alu_test.csv"; --! Output file path (prefixed since VHDL can't create folders).
...
variable a_in_v:std_logic_vector(32 - 1 downto 0); --! ALU data input A.
variable b_in_v:std_logic_vector(32 - 1 downto 0); --! ALU data input B.
variable carry_in_v:std_logic; --! ALU carry in.
variable function_in_v:std_logic_vector( 3 downto 0); --! ALU function selector.
variable c_out_v:std_logic_vector(32 - 1 downto 0); --! ALU result output C.
variable status_out_v:std_logic_vector( 4 downto 0); --! ALU status output.
...
--------------------------------------
    ...
    begin
        ...
        write(stream_out_v, string'("-- Simulation result for 'alu_test' with input file '" & PATH_FILE_IN & "'."));
...
--------------------------------------
        ...
        if test_count_v = passed_count_v then
            -- Report a passed test to output file.
            write(stream_out_v, string'("-- All tests passed for 'alu'."));
            writeline(file_out, stream_out_v);
        else
            -- Report a failed test to output file.
            write(stream_out_v, string'("-- Tests failed for 'alu_test'."));
            writeline(file_out, stream_out_v);
