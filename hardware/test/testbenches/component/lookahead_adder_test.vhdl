--! @file
--! @brief lookahead_adder testbench.
...
entity lookahead_adder_test is
end entity lookahead_adder_test;

--! @brief Default lookahead_adder test behavior.
--! @details Tests the lookahead_adder component given the input csv file.
--! @details Outputs test results to the output file.
architecture behavioral of lookahead_adder_test is

--------------------------------------
-- Start of tgen generated code block.

signal a_in_s:std_logic_vector(32 - 1 downto 0);--! Data input a.
signal b_in_s:std_logic_vector(32 - 1 downto 0);--! Data input b.
signal carry_in_s:std_logic;--! Carry input.
signal c_out_s:std_logic_vector(32 - 1 downto 0);--! Data output c.
signal carry_out_s:std_logic;--! Carry output.
...
--------------------------------------
    ...
    constant PATH_FILE_IN  : string := "../test/in/component/lookahead_adder_test.csv"; --! Input file path.
    constant PATH_FILE_OUT : string := "../test/out/component_lookahead_adder_test.csv"; --! Output file path (prefixed since VHDL can't create folders).
...
begin
    ...
    uut: entity work.lookahead_adder(behavioral)
    generic map
    (
...
variable a_in_v:std_logic_vector(32 - 1 downto 0); --! Data input a.
variable b_in_v:std_logic_vector(32 - 1 downto 0); --! Data input b.
variable carry_in_v:std_logic; --! Carry input.
variable c_out_v:std_logic_vector(32 - 1 downto 0); --! Data output c.
variable carry_out_v:std_logic; --! Carry output.
...
--------------------------------------
    ...
    begin
        ...
        write(stream_out_v, string'("-- Simulation result for 'lookahead_adder_test' with input file '" & PATH_FILE_IN & "'."));
...
read(vector_v, b_in_v);
read(vector_v, separator_v); -- Read the comma to the separator variable to discard it.
read(vector_v, carry_in_v);
read(vector_v, separator_v); -- Read the comma to the separator variable to discard it.
read(vector_v, c_out_v);
read(vector_v, separator_v); -- Read the comma to the separator variable to discard it.
read(vector_v, carry_out_v);
...
write(stream_out_v, string'(","));
write(stream_out_v, b_in_s);
write(stream_out_v, string'(","));
write(stream_out_v, carry_in_s);
write(stream_out_v, string'(","));
write(stream_out_v, c_out_s);
write(stream_out_v, string'(","));
write(stream_out_v, carry_out_s);

--  End of tgen generated code block.
--------------------------------------

            -- Check actual outputs against the expected ones.
            if c_out_s = c_out_v and carry_out_s = carry_out_v then
...
write(stream_out_v, string'("."));
write(stream_out_v, string'(" Expected carry_out: "));
write(stream_out_v, carry_out_v );
write(stream_out_v, string'("."));
...
--------------------------------------
        ...
        if test_count_v = passed_count_v then
            -- Report a passed test to output file.
            write(stream_out_v, string'("-- All tests passed for 'lookahead_adder'."));
            writeline(file_out, stream_out_v);
        else
            -- Report a failed test to output file.
            write(stream_out_v, string'("-- Tests failed for 'lookahead_adder_test'."));
            writeline(file_out, stream_out_v);
